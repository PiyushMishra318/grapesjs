...
export default {
  run(ed, s, opts = {}) {
    ...
    components.forEach(component => {
      ...
      if (!component.getClasses().includes('gjs-invisible')) component.addClass('gjs-invisible');
      else {
        flag = 0;
        component.removeClass('gjs-invisible');
      }
      let updateTb = component.get('toolbar') || [];
      updateTb = updateTb.map(tb => {
        if (tb.command === 'tlb-visibility') {
          return {
            ...tb,
            label: flag
      ...
      component.set('toolbar', updateTb);
    });
